...
class RayShootingExtractor(BaseExtractor):
    ...
    :param num_rays: Hard ceiling on the number of rays sampled (a safety
        cap).  The relative-improvement stop usually fires first; for long
        high-D scans raise this together with ``max_seconds``.  Default
        ``2_000_000``.
    :param max_seconds: Optional wall-clock budget (seconds) for the whole
        shoot.  ``None`` (default) = no time cap; the relative stop and
        ``num_rays`` govern.  Set this (e.g. for a multi-hour high-D scan)
        so the run uses its time productively and stops early only when
        saturated.
    :param max_coord: Each ray direction is sampled uniformly from
        ``[-max_coord, max_coord]^D \\ {0}``.
    :param batch_size: Rays are shot in batches of this size; the marginal
        gain is assessed after each batch.
    :param rel_improvement: Stop once a batch's **marginal gain** --
        ``new_cells / total_found_so_far`` -- stays below this fraction for
        ``plateau_patience`` consecutive batches.  Scale-invariant across
        dimensions (a fixed % of the running total, not an absolute count),
        so it adapts from D=3 to D=11.  Default ``5e-4`` (stop at ~0.05%
        marginal gain).  ``0`` disables early stopping.
    :param plateau_patience: Number of *consecutive* sub-``rel_improvement``
        batches required before stopping (default ``3``).  The yield tail
        is lumpy, so a single low batch must not end the search.
    ...
    :raises ValueError: For non-positive ``num_rays``/``max_coord``/
        ``batch_size``/``max_seconds``, a ``rel_improvement`` outside
        ``[0, 1]``, or ``plateau_patience < 1``.
    """
    ...
    def __init__(
        ...
        *,
        ...
        if not 0.0 <= rel_improvement <= 1.0:
            raise ValueError(
                f"rel_improvement must be in [0, 1], got {rel_improvement}"
            )
        ...
        *,
        ...
        while shot < self.num_rays:
            ...
            if deadline is not None and now > deadline:
                break
            if self.max_seconds is not None and now - t_start > self.max_seconds:
                break
            ...
            new = len(out) - before
            # Relative-improvement stop: bail after ``plateau_patience``
            # *consecutive* batches whose MARGINAL gain (``new / total found``)
            # falls below ``rel_improvement``.  A relative fraction is
            # scale-invariant: it tracks diminishing returns the same way at
            # D=3 (hundreds of cells) and D=11 (millions), unlike an absolute
            # per-ray floor -- which never fires when the cell count is huge,
            # so high-D runs would just burn the whole ray budget regardless.
            # The tail is lumpy, so we require a *sustained* low streak.
            if self.rel_improvement > 0.0 and shot >= self.batch_size and out:
                if new < self.rel_improvement * len(out):
                    low_streak += 1
