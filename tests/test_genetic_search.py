...
class TestParallelEval:
    """_eval_population_parallel: main-process orchestration (dispatch / dedupe /
    Case-A reuse) with an in-process dummy pool and a stubbed walk."""

    class _DummyPool:
        """Runs pool.map in-process so the orchestration can be tested without
        spawning real worker processes."""
        def map(self, fn, args):
            return [fn(a) for a in args]

    def _make_ctx(self, shard, constant):
        from dreamer.search.methods.flatland.geometry import FlatlandGeometry
    ...
    def _stub_walk(self, monkeypatch):
        ...
        monkeypatch.setattr(gs, "_pool_walk", fake_pool_walk)
    ...
    def test_case_a_skips_dispatch(self, whole_space_shard, monkeypatch):
        ...
        deltas = method._eval_population_parallel([z.copy()], ctx, self._DummyPool())
        assert deltas == [42.0]
    ...
    def test_invalid_genome_is_neg_inf(self, simple_shard, monkeypatch):
        ...
        self._stub_walk(monkeypatch)
        method = GeneticSearch(simple_shard, e, use_LIReC=False)
        ctx, sink_items = self._make_ctx(simple_shard, e)
        d = ctx["geom"].d_flat
        bad = np.array([-5, -5], dtype=np.int64)[:d]
        deltas = method._eval_population_parallel([bad], ctx, self._DummyPool())
        assert deltas == [float("-inf")]
        assert sink_items == []
